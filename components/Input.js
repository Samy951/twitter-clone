...
function Input () {
    ...
    return  (
        ...
        <div className={`border-b border-gray-700 p-3 flex space-x-3 overflow-y-scroll scrollbar-hide ${loading && "opacity-60"}`}>
            <Image src={session?.user?.image} width={40} height={40} alt={"User_avatar"} className={"h-11 w-11 rounded-full cursor-pointer"}/>
            <div className={"w-full divide-y divide-gray-700"}>
                <div className={`${selectedFile && "pb-7"} ${input && "space-y-2.5"}`}>
                    <textarea value={input}
                              onChange={(e) => setInput(e.target.value)}
                              rows={2}
                              placeholder={"What's happening"} className={"bg-transparent outline-none text-[#d9d9d9] text-lg placeholder-gray-500 tracking-wide w-full min-h-[50px]"}/>
                    {selectedFile && (
                ...
                {!loading &&(
                    <div className={"flex items-center justify-between pt-2.5"}>
                        <div className={"flex items-center"}>
                            ...
                            <div className={"absolute mt-[465px] ml-[-40] max-w-[320px] rounded-[20px]"}>
                                {showEmojis && (
                                    <Picker  data={data}
